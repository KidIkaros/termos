...
/// Handle playback controls when the focused pane is a video.
/// Space = toggle play/pause, Left/Right = seek ±5s, Up/Down = volume,
/// l = toggle loop, q = close video pane.
fn handle_video_playback(os: &mut Os, key: &KeyEvent) -> KeyResult {
    let idx = match os.focused_window {
        Some(i) => i,
        None => return KeyResult::Passthrough,
    };
    match key.code {
        KeyCode::Char(' ') => {
            // Toggle play/pause.
            if let Some(win) = os.windows.get_mut(idx) {
        ...
        KeyCode::Up | KeyCode::Char('k') => {
            ...
            if let Some(win) = os.windows.get_mut(idx) {
                if let Some(ref mut vs) = win.video_state {
                    vs.volume = (vs.volume + 0.1).min(2.0);
                }
            }
            KeyResult::Consumed
        }
        KeyCode::Down | KeyCode::Char('j') => {
            ...
            if let Some(win) = os.windows.get_mut(idx) {
                if let Some(ref mut vs) = win.video_state {
                    vs.volume = (vs.volume - 0.1).max(0.0);
                }
            }
            KeyResult::Consumed
        }
        KeyCode::Char('r') => {
            ...
            if let Some(win) = os.windows.get_mut(idx) {
                if let Some(ref mut vs) = win.video_state {
                    vs.loop_playback = !vs.loop_playback;
                    if let Some(ref decoder) = vs.decoder {
                        decoder.set_loop(vs.loop_playback);
                    }
                }
            }
            KeyResult::Consumed
        ...
        KeyCode::Char('q') => {
            // Close the video pane.
            os.close_focused_window();
            KeyResult::Consumed
        }
        _ => {
            // Pass through to shell for everything else.
...
fn handle_terminal_mode(os: &mut Os, key: &KeyEvent) -> KeyResult {
    // Video playback controls: intercept before shell passthrough.
    if os.focused_window.is_some() {
        let idx = os.focused_window.unwrap();
        if let Some(win) = os.windows.get(idx) {
            if win.pane_kind == crate::terminal::window::PaneKind::Video {
                return handle_video_playback(os, key);
            }
        }
    }
...
#[cfg(test)]
mod video_playback_tests {
    use super::*;
    use crate::terminal::window::{PaneKind, VideoState};

    fn os_with_video() -> Os {
        let config = crate::config::userconfig::UserConfig::default();
        let mut os = crate::app::Os::new(config);
        let wake = Box::new(|| {});
        let shell = os.default_shell();
        os.spawn_window(&shell, wake).unwrap();
        let idx = os.windows.len() - 1;
        os.windows[idx].pane_kind = PaneKind::Video;
        let mut vs = VideoState::default();
        vs.playing = true;
        os.windows[idx].video_state = Some(Box::new(vs));
        os.focused_window = Some(idx);
        os.mode = Mode::Terminal; // video controls only work in terminal mode
        os
    }

    #[test]
    fn space_toggles_pause() {
        let mut os = os_with_video();
        assert!(os.windows[0].video_state.as_ref().unwrap().playing);
        let key = KeyEvent::new(KeyCode::Char(' '), KeyModifiers::NONE);
        handle_key(&mut os, &key);
        assert!(!os.windows[0].video_state.as_ref().unwrap().playing);
        handle_key(&mut os, &key);
        assert!(os.windows[0].video_state.as_ref().unwrap().playing);
    }
    ...
    fn r_toggles_loop() {
        let mut os = os_with_video();
        assert!(!os.windows[0].video_state.as_ref().unwrap().loop_playback);
    ...
    #[test]
    fn arrow_left_seek_backward() {
        let mut os = os_with_video();
        let key = KeyEvent::new(KeyCode::Left, KeyModifiers::NONE);
        handle_key(&mut os, &key);
        assert!(os.windows[0].video_state.as_ref().unwrap().current_secs >= 0.0);
    }

    #[test]
    fn arrow_right_seek_forward() {
        let mut os = os_with_video();
        let key = KeyEvent::new(KeyCode::Right, KeyModifiers::NONE);
        handle_key(&mut os, &key);
        assert!(os.windows[0].video_state.as_ref().unwrap().current_secs > 0.0);
    ...
    #[test]
    fn up_down_adjust_volume() {
        let mut os = os_with_video();
        let vol_before = os.windows[0].video_state.as_ref().unwrap().volume;
        let up = KeyEvent::new(KeyCode::Up, KeyModifiers::NONE);
        handle_key(&mut os, &up);
        assert!(os.windows[0].video_state.as_ref().unwrap().volume > vol_before);
        let down = KeyEvent::new(KeyCode::Down, KeyModifiers::NONE);
        handle_key(&mut os, &down);
        handle_key(&mut os, &down);
        assert!(os.windows[0].video_state.as_ref().unwrap().volume < vol_before);
    }

    #[test]
    fn q_closes_video_pane() {
        let mut os = os_with_video();
        assert_eq!(os.windows.len(), 1);
        let key = KeyEvent::new(KeyCode::Char('q'), KeyModifiers::NONE);
        handle_key(&mut os, &key);
        assert!(os.windows.is_empty() || os.windows.iter().all(|w| w.exited));
    }
}
