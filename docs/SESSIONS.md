...
TermOS runs in one of two modes: a local session that lives and dies with the
...
Running `termos` with no subcommand starts a local session. Everything, the
...
A daemon session lives in a separate `termos` daemon process. The daemon owns the
...
```bash
termos new mysession          # create a persistent session and attach to it
termos new mysession --detach # create it headless, attach later
termos attach mysession       # attach to an existing session
termos attach                 # attach to the most recent session
termos attach mysession -c    # attach, creating the session if it is missing
termos ls                     # list live sessions
termos ls --json              # the same list, machine readable
termos kill-session mysession # terminate a session and all its windows
```
...
```bash
termos daemon                 # run in the foreground (useful for debugging)
termos daemon --log-level=messages
termos kill-server            # stop the daemon and all its sessions
```

`termos kill-server` is synchronous. It returns only after every session's state
...
```
-- termos: session restored, fresh shell in /home/you/project --
```

Restored shells get `TERMOS_RESTORED=1` in their environment, so your shell rc can
...
session shows a `restored` tag in `termos ls`, in the sidebar and in the session
switcher, and `termos attach` says so before it hands over the screen:
...
is left on disk and can still be restored on demand with `termos resurrect`.

A session killed with `termos kill-session` has its saved state deleted, because
...
If a state file is corrupt, or was written by a newer TermOS whose format this
...
```bash
termos resurrect              # list the sessions that can be restored
termos resurrect mysession    # restore that session and attach to it
```

With no arguments, `termos resurrect` prints a table of every saved session with
...
newer TermOS. In the last two cases it also prints where the file was archived.

## Where State Lives

| What | Path |
|---|---|
| Saved session state | `$XDG_STATE_HOME/termos/sessions/<name>.json` (typically `~/.local/state/termos/sessions/`) |
| Archived bad state | `$XDG_STATE_HOME/termos/sessions/archive/`, pruned after 14 days |
| Daemon socket | `$XDG_RUNTIME_DIR/termos/termos.sock`, falling back to `/tmp/termos-<uid>/termos.sock` |
