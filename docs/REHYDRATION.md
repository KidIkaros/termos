...
**The snapshot.** `terminal_state_of` (`src/session/mod.rs`) serializes the
...
and up to 1000 scrollback rows. `apply_terminal_state` reads it back, and
`restore_terminal_content` (`src/app/session.rs`) is the window around
...
restoring lived in `src/app`, which is how they came to disagree about which
...
(`Pty::append_to_buffer`) and a monotonic `output_seq` counting every byte ever
produced. `Pty::subscribe(client_id, from_seq)` replays the ring from `from_seq`
onward and then streams live. `Pty::unsubscribe` returns the position the client
reached, which the daemon parks in `conn_state.pty_resume`. The stream is history:
...
(`SubscribePtyPayload.from_seq`). That pairing is what keeps them from
overlapping.
...
`test_rehydration_matrix` (`tests/rehydration_matrix.rs`) proves a
...
`test_wire_carries_the_whole_cell` (`tests/wire_fidelity.rs`) proves the
*wire*, and it exists because the matrix structurally cannot. The matrix
...
`tests/session_switch_fidelity.rs` is the third rung: a real client in a
...
cannot hold. `test_resize_seam_stays_closed` resizes a pane that produces little
...
rows to keep its own seam. `test_saturated_switch_no_resize` takes a pane already
...
**M1 - `prime_pane_from_daemon`.** Snapshot, then subscribe with whatever
`pty_resume` the daemon still holds for this pane.

**M2 - `restore_terminal_states` then `setup_pty_output_handlers`.** Snapshot for
every window in the session, then subscribe the current workspace's panes. Every
route that reaches M2 has been through `handle_detach`, which clears `pty_resume`
...
`tests/rehydration_matrix.rs` reproduced all three:
...
- Output already queued for a client's emulator is applied before the pane is
  primed (`Window::drain_pending_output`), not discarded. It is older than the
...
- **The request's own fields are honoured.** `include_scrollback` was written by
  the client and never read; `max_scrollback_lines` was never written or read; and
...
- **Scroll position is not on the wire, by design.** `Window.scrollback_offset`
  and `CopyMode.scroll_offset` are where *this viewer* is looking, not properties
...
- **A colour keeps its encoding.** `CellState` flattened every colour through
  ...
  in shades the user never chose. `color_to_wire` keeps the three encodings apart,
  and `color_from_wire` resolves a palette entry through the emulator that will
...
told the daemon over the client resize path, which is fire-and-forget. Everything
...
and stayed that way: `test_rehydration_matrix/workspace-switch/scrolled-back` saw a
...
- `Pty::resize` does not touch the daemon's emulator. It queues the resize on
  the write channel, behind the bytes already read, and broadcasts it to every
  subscriber under the same lock the output reader appends and broadcasts under. One
...
- The daemon announces it as a PTY-resized message, which ends the output batch in
...
- The client queues it into the same channel its output goes through
  (`Window::resize_from_stream`), so it is applied in the order it arrived.
- `Window::resize` stays the one choke point for the announcement, `announced_w/h`
  and SIGWINCH. Only the emulator grid moved, and only while `stream_owns_size` is
...
- A restore discards queued output, and a queued resize with it, so
  `prime_pane_from_daemon` takes the emulator to the snapshot's own bounds. That is
...
now leaves a mark at its stream position (`resize_marks`), pruned with the ring,
...
`get_terminal_state` reports the emulator's own size rather than the pane's
...
`Window::resize_visual` still resizes a streamed pane's emulator directly. It is
...
`prime_pane_from_daemon` announces a pane's size and re-fetches the snapshot
...
`apply_terminal_state` seeds a surviving emulator with the lines that scrolled
...
`get_modes` drops because they share an int keyspace with the DEC modes. Insert
