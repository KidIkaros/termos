...
The showkeys overlay displays your keyboard input on screen in real-time, making TermOS ideal for presentations, screencasts, and live coding demos. When enabled, every key you press appears in a clean, unobtrusive overlay at the bottom of the screen.
...
- Action names (when a key triggers a TermOS action)
- Key repeat counts
- Timing information
...
Enable showkeys when launching TermOS:
...
```bash
termos ssh --show-keys --host 0.0.0.0 --port 2222
```

### Toggle During Runtime

While TermOS is running, toggle showkeys:
...
Pass the flag to termos-web:

```bash
termos-web --show-keys --port 7681
```
...
When a key triggers a TermOS action, the action name appears:
...
TermOS shows the last 5 keystrokes by default. Older keys slide out as new ones arrive:
...
Keys appear in rounded boxes with theming that matches your current TermOS theme:
...
```bash
# Before presentation
termos new presentation --show-keys --theme nord
```
...
When teaching new developers how to use TermOS or terminal workflows:

```bash
termos --show-keys --theme github
```
...
```bash
termos --show-keys --debug
```

Watch the overlay to confirm TermOS receives the correct key events and resolves them to the expected actions.

## Configuration

### History Size

The default ring buffer shows 5 keys. This is currently hardcoded in the app struct (`KEY_HISTORY_MAX_SIZE`). To change it, you would need to modify `src/app/mod.rs`.
...
```bash
termos --theme dracula --show-keys      # Purple/pink theme
termos --theme nord --show-keys         # Blue/arctic theme
```
...
Showkeys hooks into the input handling pipeline at `src/keys.rs`. Every key event passes through the showkeys recorder before normal processing.
...
```rust
pub struct KeyEvent {
    pub key: String,         // "a", "Ctrl+C", "Enter"
    pub modifiers: Vec<String>,  // ["Ctrl", "Shift"]
    pub timestamp: Instant,  // When pressed
    pub count: usize,        // Repeat count
    pub action: String,      // "new_window", "toggle_tiling"
}
```
...
```rust
pub show_keys: bool,              // Enabled/disabled
pub recent_keys: Vec<KeyEvent>,   // Ring buffer
pub key_history_max_size: usize,  // Buffer size (default: 5)
```

### Rendering

The showkeys overlay renders in `src/app/render_overlays.rs` as a separate layer composited on top of the main view. It's drawn after windows but before notifications.
...
Not all keys map to TermOS actions. Only keybindings configured in your config file show action names.
...
```bash
# Start TermOS with showkeys and a clean theme
termos --show-keys --theme github

# Start recording with asciinema
asciinema rec termos-demo.cast
...
termos new workshop --show-keys --theme nord
...
```bash
# Instructor terminal
termos --show-keys --theme dracula
