...
TermOS supports multiple clients connecting to the same session simultaneously, enabling collaborative workflows and flexible access patterns.
...
Multi-client mode allows multiple terminal instances (SSH, web, or local TUI clients) to attach to the same TermOS session concurrently. All clients see the same windows, input, and output in real-time.
...
developer-a$ termos attach pairing-session

# Developer B joins the same session
developer-b$ ssh remote-server
remote$ termos attach pairing-session
```
...
user$ termos attach support-session

# Support engineer connects
support$ termos attach support-session
```
...
```bash
# Work laptop
laptop$ termos attach dev-work

# Later, from home desktop
desktop$ ssh work-server
work-server$ termos attach dev-work
```
...
```bash
# Presenter
presenter$ termos attach demo

# Multiple viewers (via SSH or web)
viewer1$ termos attach demo
viewer2$ termos-web  # Navigate to session "demo" in browser
```
...
To handle different client terminal sizes, TermOS uses the **minimum size** strategy:
...
termos attach my-session --no-create
```
...
termos send-keys --session my-session "ctrl+b" "d"
```

### Kill Session
```bash
# Kill specific session
termos kill my-session
...
```bash
# Terminal 1 (local)
# Note: daemon starts automatically with 'termos new' or 'termos attach'
local$ termos new dev
# Or attach to existing session (creates if doesn't exist):
# local$ termos attach dev

# Terminal 2 (SSH from remote)
remote$ ssh myserver
myserver$ termos attach dev
```
...
```bash
# Client 1 (local TUI)
local$ termos attach collab

# Client 2 (SSH)
ssh$ termos attach collab
...
- **CPU:** Daemon uses efficient multi-client I/O
...
termos new mysession --log-level=messages
```
...
**Solution:**
- Detach and reattach: `Ctrl+B, d` then `termos attach <session>`
...
termos list-windows --json 2>&1 | grep -q "success" && echo "Daemon running" || echo "Daemon not running"
```

## Comparison with tmux/screen

| Feature | TermOS | tmux | screen |
|---------|-------|------|--------|
| Multi-client | ✅ Yes | ✅ Yes | ✅ Yes |
| Size strategy | Minimum | Minimum | Minimum |
| Real-time sync | ✅ Yes | ✅ Yes | ✅ Yes |
| Web clients | ✅ Yes (termos-web) | ❌ No (external tools) | ❌ No |
...
```bash
# Trainer
trainer$ termos attach training
trainer$ # Demonstrates commands

# 5 trainees join
trainee1$ termos attach training
trainee2$ termos attach training
# ... etc
...
prod-server$ termos attach incident-debug
...
- [`termos ssh`](CLI_REFERENCE.md#termos-ssh) - SSH-based multi-client access
