...
TermOS supports user-configurable keybindings through a TOML configuration file, following the XDG Base Directory specification.
...
**Note**: Many system constants (window sizes, animation speeds, refresh rates) are currently hardcoded in `src/config/constants.rs` and cannot be configured via TOML.
...
termos keybinds list

# View only your customizations
termos keybinds list-custom
```
...
**Default path:** `~/.config/termos/config.toml`

On first launch, TermOS automatically creates a default configuration file. The exact location follows the XDG Base Directory specification:

- Linux/macOS: `~/.config/termos/config.toml`
- Custom: `$XDG_CONFIG_HOME/termos/config.toml` (if `XDG_CONFIG_HOME` is set)
...
You only need to specify what you want to customize. TermOS automatically fills in missing keybindings with defaults:

```toml
# ~/.config/termos/config.toml
...
The `[appearance]` section controls the visual presentation of TermOS.
...
`~/.config/termos/themes/` can be named here exactly like built-in ones. Leave it
...
What TermOS does when a pane's agent state changes. Every key can be turned off,
...
dock = true             # the clickable message in TermOS's own dock
...
is in front of you, including over `termos ssh` and inside tmux (which needs
`set -g allow-passthrough on`). This is deliberately not a desktop notification
raised by TermOS itself: with `termos ssh` the interface runs on the *remote* host,
...
it. Over `termos ssh` that means it comes out of your laptop, not the host the
...
Setting `TERMOS_NO_SOUND` in the environment silences the cues however TermOS is
...
[HOOKS.md](HOOKS.md). It is how you wire a notifier TermOS does not ship - ntfy,
Pushover, a webhook, or `notify-send` on a machine that actually has a display:

```toml
[notifications.agent]
command = 'curl -s -d "$TERMOS_WINDOW_NAME is $TERMOS_AGENT_STATE" ntfy.sh/my-topic'
```
...
```toml
[hooks]
after-new-window = "notify-send 'TermOS' 'new window'"
```
...
The `[tape]` table controls per-directory project tapes (`.termos.tape`). When the
focused shell enters a directory that carries one, TermOS can build a project
...
`TERMOS_TAPE_AUTORUN` overrides this for a single run. An untrusted tape is inert:
...
In `[keybindings.terminal_mode]` an unbound key is handed back to the shell in the pane. This matters most for `alt+left` and `alt+right`, which TermOS binds to directional pane focus and which readline, fish and zsh use for word-wise cursor movement:
...
On macOS, TermOS supports the Option key (displayed as "opt" or "option" on Mac keyboards).
...
**Key expansion:** When you use `opt+1`, TermOS automatically handles:
...
If the same key is bound to multiple actions, TermOS will warn you during startup. The last binding takes precedence.

View conflicts:
```bash
termos keybinds list | grep <your-key>
```
...
2. Quit TermOS (press `q` in window management mode)
3. Restart TermOS
