# Sip - Serve TUI Apps in the Browser
...
Sip is a Go library that serves any Bubble Tea application through a web browser. It provides the web terminal infrastructure that powers `termos-web`, extracted into a reusable package for the entire Bubble Tea ecosystem.

> **Note:** Sip is a Go library from the upstream TUIOS project. The TermOS Rust port uses its own web server implementation (see [WEB.md](WEB.md)), but the upstream sip library remains the reference for the web terminal architecture.
...
- [TUIOS](https://github.com/Gaurav-Gosain/tuios) - Terminal window manager using sip (upstream Go project)
