...
TermOS implements automatic window tiling using Binary Space Partitioning (BSP), an algorithm that recursively divides screen space to fit windows efficiently. Unlike rigid grid-based tiling, BSP adapts to any number of windows and allows fine-grained control over splits.
...
BSP tiling automatically arranges windows in a tree structure where each node represents either a window or a split (horizontal or vertical). When you create a new window, TermOS intelligently decides where to place it based on the current layout.
...
When tiling is enabled, TermOS automatically positions new windows:
...
Internally, TermOS maintains a binary tree for each workspace:
...
Without preselection, TermOS uses its default spiral algorithm. With preselection, you override this for precise control.
...
Resizing modifies the split ratio at the nearest split node. TermOS walks up the tree to find the appropriate split to adjust.
...
When you manually create splits and preselect positions, TermOS marks the workspace as having a "custom layout". This layout persists even when you close windows.
...
Default scheme: `AutoScheme` set to spiral pattern.
...
After tree modifications, TermOS walks the tree recursively, calculating rectangles:
...
- `workspace_trees[workspace_num]` - BSP tree structure
- `workspace_master_ratio[workspace_num]` - Master ratio for resizing
- `workspace_has_custom[workspace_num]` - Whether user made manual splits
...
- [Architecture Guide](ARCHITECTURE.md) - Technical details on BSP implementation (see `src/layout/bsp.rs`)

## Comparison with Other Tiling Algorithms

### BSP vs. Grid

**Grid tiling (not used in TermOS):**
...
Some window managers require you to manually tile every window. TermOS offers both:
